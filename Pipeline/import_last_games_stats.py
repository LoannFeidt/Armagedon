...
def get_games(cur):
    cur.execute("""SELECT id, home_team, away_team, status
        FROM public.games
        WHERE date < CURRENT_DATE - 1 AND status = 1
        ORDER BY date DESC """)
    return cur.fetchall()
...
if __name__ == "__main__":
    ...
    conn_db.commit()
    print(f"{count_add} stats(s) added")
    print(f"{count_cleanse} stats(s) cleansed   ")
    cur.close()
    conn_db.close()
